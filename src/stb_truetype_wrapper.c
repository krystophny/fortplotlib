...
/*
 * Vertex shape functions - get vertex data for outlines
 */

/* Convert stbtt_vertex to Fortran-compatible ttf_vertex_t structure */
typedef struct {
...
/*
 * Get codepoint shape as vertex array
 * Returns number of vertices (similar to stbtt_GetCodepointShape)
 */
int stb_wrapper_get_codepoint_shape(stb_fontinfo_wrapper_t *wrapper, int codepoint, fortran_vertex_t **vertices) {
    if (!wrapper || !wrapper->private_data || !vertices) {
        return 0;
    }

    stb_font_context_t *context = (stb_font_context_t*)wrapper->private_data;
    stbtt_vertex *stb_vertices = NULL;

    /* Get vertices from STB */
    int num_vertices = stbtt_GetCodepointShape(&context->font_info, codepoint, &stb_vertices);
    ...
    /* Allocate Fortran-compatible vertex array */
    fortran_vertex_t *fort_vertices = (fortran_vertex_t*)malloc(num_vertices * sizeof(fortran_vertex_t));
    if (!fort_vertices) {
        stbtt_FreeShape(&context->font_info, stb_vertices);
        *vertices = NULL;
        return 0;
    }

    /* Convert STB vertices to Fortran format */
    for (int i = 0; i < num_vertices; i++) {
        fort_vertices[i].x = (int)stb_vertices[i].x;
        fort_vertices[i].y = (int)stb_vertices[i].y;
        fort_vertices[i].cx = (int)stb_vertices[i].cx;
        fort_vertices[i].cy = (int)stb_vertices[i].cy;
        fort_vertices[i].cx1 = (int)stb_vertices[i].cx1;
        fort_vertices[i].cy1 = (int)stb_vertices[i].cy1;
        fort_vertices[i].type = (int)stb_vertices[i].type;
    }

    /* Free STB vertices */
    stbtt_FreeShape(&context->font_info, stb_vertices);

    *vertices = fort_vertices;
    return num_vertices;
}

/*
 * Get glyph shape as vertex array
 * Returns number of vertices (similar to stbtt_GetGlyphShape)
 */
int stb_wrapper_get_glyph_shape(stb_fontinfo_wrapper_t *wrapper, int glyph_index, fortran_vertex_t **vertices) {
    if (!wrapper || !wrapper->private_data || !vertices) {
        return 0;
    }
    ...
    /* Get vertices from STB */
    int num_vertices = stbtt_GetGlyphShape(&context->font_info, glyph_index, &stb_vertices);
...
/*
 * Free shape vertex array
 */
void stb_wrapper_free_shape(fortran_vertex_t *vertices) {
    if (vertices) {
        free(vertices);
    }
}
