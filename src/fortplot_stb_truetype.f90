module fortplot_stb_truetype
    ...
    public :: stb_get_codepoint_shape, stb_get_glyph_shape, stb_free_shape
    ...
    interface
        ...
        function stb_wrapper_get_codepoint_shape(font_info, codepoint, vertices) &
                 bind(C, name="stb_wrapper_get_codepoint_shape")
            import :: c_int, stb_fontinfo_t, c_ptr
            type(stb_fontinfo_t), intent(in) :: font_info
            integer(c_int), value :: codepoint
            type(c_ptr), intent(out) :: vertices
            integer(c_int) :: stb_wrapper_get_codepoint_shape
        end function stb_wrapper_get_codepoint_shape

        function stb_wrapper_get_glyph_shape(font_info, glyph_index, vertices) &
                 bind(C, name="stb_wrapper_get_glyph_shape")
            import :: c_int, stb_fontinfo_t, c_ptr
            type(stb_fontinfo_t), intent(in) :: font_info
            integer(c_int), value :: glyph_index
            type(c_ptr), intent(out) :: vertices
            integer(c_int) :: stb_wrapper_get_glyph_shape
        end function stb_wrapper_get_glyph_shape

        subroutine stb_wrapper_free_shape(vertices) bind(C, name="stb_wrapper_free_shape")
            import :: c_ptr
            type(c_ptr), value :: vertices
        end subroutine stb_wrapper_free_shape
...
contains
    ...
    function stb_get_codepoint_shape(font_info, codepoint, num_vertices) result(vertices_ptr)
        !! Get vertex shape data for a codepoint (STB C implementation)
        type(stb_fontinfo_t), intent(in) :: font_info
        integer, intent(in) :: codepoint
        integer, intent(out) :: num_vertices
        type(c_ptr) :: vertices_ptr

        num_vertices = stb_wrapper_get_codepoint_shape(font_info, int(codepoint, c_int), vertices_ptr)
    end function stb_get_codepoint_shape

    function stb_get_glyph_shape(font_info, glyph_index, num_vertices) result(vertices_ptr)
        !! Get vertex shape data for a glyph index (STB C implementation)
        type(stb_fontinfo_t), intent(in) :: font_info
        integer, intent(in) :: glyph_index
        integer, intent(out) :: num_vertices
        type(c_ptr) :: vertices_ptr

        num_vertices = stb_wrapper_get_glyph_shape(font_info, int(glyph_index, c_int), vertices_ptr)
    end function stb_get_glyph_shape

    subroutine stb_free_shape(vertices_ptr)
        !! Free vertex shape data allocated by STB C
        type(c_ptr), intent(in) :: vertices_ptr

        call stb_wrapper_free_shape(vertices_ptr)
    end subroutine stb_free_shape
