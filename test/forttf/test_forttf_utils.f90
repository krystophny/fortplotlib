...
contains
    ...
    subroutine export_pgm_bitmap(pixels, width, height, filename)
        !! Export bitmap as PGM (Portable Gray Map) file for visual comparison
        !! Handles signed int8 pixel values correctly (converts to 0-255 range)
        integer(c_int8_t), intent(in) :: pixels(:)
        integer, intent(in) :: width, height
        character(len=*), intent(in) :: filename
        
        integer :: unit, i, j, pixel_val
        
        ! Open file for writing
        open(newunit=unit, file=filename, status='replace', action='write')
        
        ! Write PGM header
        write(unit, '(A)') 'P2'  ! ASCII grayscale
        write(unit, '(A)') '# Generated by ForTTF test'
        write(unit, '(I0,A,I0)') width, ' ', height
        write(unit, '(A)') '255'  ! Max gray value
        
        ! Write pixel data (convert signed int8 to unsigned 0-255)
        do j = 0, height-1
            do i = 0, width-1
                pixel_val = int(pixels(j * width + i + 1))
                if (pixel_val < 0) pixel_val = pixel_val + 256
                write(unit, '(I0,A)', advance='no') pixel_val, ' '
            end do
            write(unit, *)  ! New line
        end do
    ...
    end subroutine export_pgm_bitmap
