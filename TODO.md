...
- `fpm test --target test_stb_rasterization` — Run STB rasterization pipeline tests
...
- `test_bitmap_content.f90`: Compares rendered bitmap content.
- `test_character_coverage.f90`: Validates rendering for a set of characters.
- `test_stb_comparison.f90`: End-to-end comparison with STB.
- **Rasterizer Pipeline Tests:**
  - `test_stb_structures.f90`: Validates ported data structures.
  - `test_curve_flattening.f90`: Tests for Bézier curve tessellation.
  - `test_edge_processing.f90`: Tests for edge building and sorting.
  - `test_active_edges.f90`: Tests for active edge management.
  - `test_stb_rasterization.f90`: **(To be created)** for the new scanline rasterizer functions.
- **Debugging & Utility Tests:**
  - `test_forttf_utils.f90`: Tests for utility functions.
  - `test_glyph_outline.f90`: Tests for glyph outline parsing.
  - `test_simple_bitmap.f90`: Basic bitmap tests.
  - `test_debug_bitmap.f90`: Debugging tests for bitmaps.
...
**Methodology:** Each function must be ported from `thirdparty/stb_truetype.h` to its Fortran equivalent. A corresponding test must be created in `test/forttf/test_stb_rasterization.f90` to validate the output against the original C function.
...
- [ ] **Create `test/forttf/test_stb_rasterization.f90`**
...
- [ ] **Pixel-Perfect Bitmap Comparison**
  - **Test:** `test_exact_complete_pipeline_vs_stb()` in `test/forttf/test_stb_comparison.f90`
...
2.  **Develop Tests:** For each ported function, create a corresponding test in `test/forttf/test_stb_rasterization.f90` to ensure its output matches the STB reference.
